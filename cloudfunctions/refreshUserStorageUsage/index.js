...
function isLegacyHlsFile(filePath) {
  const normalizedPath = String(filePath || '').toLowerCase();
  if (!normalizedPath) return false;

  if (normalizedPath.includes('/hls_playable/')) return true;
  if (normalizedPath.includes('/hls/')) return true;
  if (normalizedPath.endsWith('.m3u8')) return true;
  if (normalizedPath.endsWith('.ts')) return true;
  if (normalizedPath.includes('/segment/')) return true;
  if (normalizedPath.includes('/segments/')) return true;
...
function shouldCountFile(openid, file) {
  ...
  if (filePath === `${openid}/.init` || filePath.endsWith('/.init')) return false;
  if (isLegacyHlsFile(filePath)) return false;
  return true;
}
